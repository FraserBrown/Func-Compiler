...
* Title: Language Processors Coursework Part 2 Func-Compiler
**/
int symb;
//#define DEBUG
...
void codeTree(NODE * t){
	...
	switch(t->tag){
		...
		case LPAREN: 	codeSignature(t->f.b.n1); 	//build function signature code
						...
						return;
		case TBEGIN:	codeCommand(t->f.b.n1);		//generate code for begin block
						codeEnd(t->f.b.n2);
						return;
...
void codeName(NODE * t){
	...
	switch(t->tag){
		...
		case COMMA:	//codeArgs(t->f.b.n1);	//did not get time to implement
					//codeArgs(t->f.b.n1);
					return;
...
/**
* generates code that assigns variables to arrays
**/
void codeAOSAssign(NODE * t){
	...
	if (t->f.b.n2->tag == LPAREN){	//if assigning built in function to array value
...
/**
*	traverses tree and generates MIPS code for while loop
**/
...
/**
*	traverses if ast and genereate mips code for if statement
**/
void codeIf(NODE * t){
	#ifdef DEBUG
		//printf("\n enter codeif node: %s\n",showSymb(t->tag)); 
...
/**
*	generate mips code for binary operations
**/
void codeBop(NODE * t){
	#ifdef DEBUG
...
void codeLess(NODE * t){
	...
	switch(t->tag){
		case LPAREN:	codeBop(t);
						return;
		case COMMA:		commaCheck("$t8", t->f.b.n1);
...
/**
* generate mips code to push to stack
**/
...
/**
* generate mips code to pop from stack
**/
...
/**
*	checks if expression is a value or is a number and alter MIPS code depending 
**/
...
/**
* generate jump MIPS code to jump back to main function after end of function is reached
**/
void codeEnd(NODE * t){
